...
function RecoveryQueue({ queuedPayloads, isLoading, refetch }: RecoveryQueueProps) {
  ...
  const payloadsByChain = queuedPayloads.reduce(
    (acc, payload) => {
    ...
    const options = [
      {
        ...
        label: (
          <div className="flex flex-row items-center gap-2">
            <Text variant="xsmall" className="text-primary/80">
              All Chains
            </Text>
          </div>
        )
  ...
  const filteredPayloadsByChain = filteredPayloads.reduce(
    (acc, payload) => {
      const chainId = payload.chainId
      if (!acc[chainId]) {
        acc[chainId] = []
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <Text variant="small" fontWeight="bold" color="text50" className="break-words">
          Recovery Payloads ({filteredPayloads.length}{' '}
          {filteredPayloads.length === 1 ? 'payload' : 'payloads'}{' '}
          {selectedChain !== 'all' ? 'filtered' : 'queued'})
        </Text>
          ...
          >
            {executedHidden ? (
              <div className="flex flex-row gap-2 items-center">
                <VisibleIcon /> Show Executed
              </div>
            ) : (
              <div className="flex flex-row gap-2 items-center">
                <HiddenIcon /> Hide Executed
              </div>
            )}
          </Button>
          <IconButton icon={RefreshIcon} variant="secondary" size="xs" onClick={() => refetch()} />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 bg-background-secondary border border-border-card rounded-xl p-2">
        {isLoading ? (
          ...
          <div className="flex flex-col">
            <Text variant="normal" fontWeight="medium" color="text50" className="text-center p-4">
              {selectedChain === 'all'
                ? 'No recovery payloads queued yet.'
                : 'No recovery payloads found for the selected chain.'}
            </Text>
          </div>
